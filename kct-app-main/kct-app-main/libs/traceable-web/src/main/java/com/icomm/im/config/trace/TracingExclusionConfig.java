...
public class TracingExclusionConfig {
    ...
    public ObservationPredicate noActuator() {
        return (name, context) -> {
            if (name.equals("http.server.requests") && context instanceof ServerRequestObservationContext srCtx) {
                return !Objects.requireNonNull(srCtx.getCarrier()).getRequestURI().contains(actuatorUrl + "/");
            }
            return true;
        };
    }
    ...
    public ObservationPredicate noSwagger() {
        return (name, context) -> {
            if (name.equals("http.server.requests") && context instanceof ServerRequestObservationContext srCtx) {
                return !Objects.requireNonNull(srCtx.getCarrier()).getRequestURI().contains("/swagger");
            }
            return true;
        };
    }
    ...
    public ObservationPredicate noApiDocs() {
        return (name, context) -> {
            if (name.equals("http.server.requests") && context instanceof ServerRequestObservationContext srCtx) {
                return !Objects.requireNonNull(srCtx.getCarrier()).getRequestURI().contains("/v3/api-docs");
            }
            return true;
        };
    }
